...
function err(thetype, field)
{
    ...
    throw new Error("Fatal error in "+thetype+", on field "+field);
}
...
function show_host(root, h)
{
    ...
    const tab1 = h['geometry'].split(/\D+/);
    if (tab1.length != 4) err("hosts id "+h['id'], "geometry parsing");
    h['w'] = parseInt(tab1[0]);
    h['h'] = parseInt(tab1[1]);
    h['x'] = parseInt(tab1[2]);
    h['y'] = parseInt(tab1[3]);
    ...
    newelem1.className = 'host_div';
    newelem1.style.position = "absolute";
    newelem1.style.width = h['w']+'px';
    newelem1.style.height = h['h']+'px';
    newelem1.style.top = h['y'] + 'px';
    newelem1.style.left = h['x'] + 'px';
    newelem1.style.backgroundImage = 'url(img/'+h['img']+')';
    root.appendChild(newelem1);

    // label + routes
    const tab2 = h['labelpos'].split(',');
    if (tab2.length != 2) err("host id "+h['id'], "labelpos parsing");
    h['lx'] = parseInt(tab2[0]);
    h['ly'] = parseInt(tab2[1]);

    let newelem2 = document.createElement('div');
    newelem2.className = 'host_info_div';
    newelem2.style.position = "absolute";
    newelem2.style.top = (h['y']+h['ly'])+'px';
    newelem2.style.left = (h['x']+h['lx'])+'px';
    let label = '<table><tr><td>'+h['type']+' '+h['id']+': <i>'+h['name']+'</id></td></tr>';
    let str = '';
    routes.forEach(r => {if (h['id'] == r['hid']) { str += '<tr><td>'+get_route_info(r)+'</td></tr>\n'; r['h'] = h;}});
    if (str != '') label += '<tr><td>Routes :</td></tr>\n'+str;
    label += '</table>';
    newelem2.innerHTML = label;
...
function get_route_info(r)
{
    ...
    let route_active = (r['route_edit'] == 'true') ? '' : 'disabled';
	let gate_active = (r['gate_edit'] == 'true') ? '' : 'disabled';
    let routestr = '<input size=15 type=text id=route_'+r['rid']+' value="'+r['route']+'" '+route_active+'> =&gt; <input size=15 type=text id=gate_'+r['rid']+' value="'+r['gate']+'" '+gate_active+'>';
...
function show_ifs(root, itf)
{
	...
	let ip_active = (itf['ip_edit'] == 'true') ? '' : 'disabled';
	let mask_active = (itf['mask_edit'] == 'true') ? '' : 'disabled';
...
function show_goals(g)
{
    ...
    let div = document.getElementById("goals_id");
...
function load_board()
{
	let g_my_login = localStorage.getItem("g_my_login");
	if (!g_my_login)
